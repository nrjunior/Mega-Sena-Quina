...
final class QuinaAction
{
    ...
    public function __invoke(Request $request, Response $response, $args)
    {
        ...
        $key = FileSystemCache::generateCacheKey('cache-feed_QuinaAction', null);
        $data = FileSystemCache::retrieve($key);

        if($data === false)
        {
            $doc = phpQuery::newDocumentFileHTML('http://g1.globo.com/loterias/quina.html');
